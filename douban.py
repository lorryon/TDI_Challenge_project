...
while True:
    time.sleep(15)
    conn = requests.get(page_url, cookies={"Cookie": "bid=%s" % "".join(random.sample(string.ascii_letters + string.digits, 11))})
    data = conn.content
 ...
 with open('douban_top_250.csv', 'w', newline='') as csvfile:
...
while movie_number < 250:
    ...
    soup = BeautifulSoup(data)
    douban_score = soup.strong.text
    douban_review_count = soup.find("span", {"property": "v:votes"}).text
    star = []
    for str in soup.find_all(attrs={"class": "rating_per"}):
        star.append(str.text)
    parsed_url = soup.find_all("a", {"target": "_blank", "rel": "nofollow"})
    if len(parsed_url) > 1:
 ...
 with open('douban_top_250_1.csv', 'w', newline='') as csvfile:
